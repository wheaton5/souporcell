...
fn call_doublets(params: &Params, cluster_allele_counts: FnvHashMap<(usize,usize),Vec<(f64,f64)>>, 
    ...
    for (cell, loci) in cell_allele_counts.iter().enumerate() {
        ...
        let cluster2 = cell_clusters[cell].1;
        ...
        for (locus, ref_count, alt_count) in loci {
            ...
            let doublet_allele_fraction = (1.0-params.p_soup)*doublet_allele_fractions[*locus] + params.p_soup*soup_allele_fractions[*locus];
            let lbetapdfsingleton = factorial::ln_binomial(*ref_count + *alt_count, *ref_count) + 
                                                beta::ln_beta(*ref_count as f64 + singleton_allele_fraction*(locus_counts.0 + locus_counts.1) + 1.0, *alt_count as f64 + (1.0 - singleton_allele_fraction)*(locus_counts.0 + locus_counts.1) + 1.0) - 
                                                beta::ln_beta(singleton_allele_fraction*(locus_counts.0+locus_counts.1) + 1.0, (1.0-singleton_allele_fraction)*(locus_counts.0 + locus_counts.1) + 1.0); //beta.pdf(singleton_allele_fraction).log10();
            let lbetapdfdoublet = factorial::ln_binomial(*ref_count + *alt_count, *ref_count) + 
                                                beta::ln_beta(*ref_count as f64 + doublet_allele_fraction*(locus_counts.0+locus_counts.1) + 1.0, *alt_count as f64 + (1.0-doublet_allele_fraction)*(locus_counts.0+locus_counts.1) + 1.0) - 
                                                beta::ln_beta(doublet_allele_fraction*(locus_counts.0+locus_counts.1) + 1.0, (1.0-doublet_allele_fraction)*(locus_counts.0+locus_counts.1) + 1.0); //beta.pdf(singleton_allele_fraction).log10();
            ...
            if debug {
                if singleton_logprob >= doublet_logprob {
                    println!("S\tcluster1 AF {}\t combined AF {}\tref {} alt {}\tS {}\tD {}",singleton_allele_fraction, doublet_allele_fraction,ref_count, alt_count, singleton_logprob, doublet_logprob);
                } else {
                    println!("D\tcluster1 AF {}\t combined AF {}\tref {} alt {}\tS {}\tD {}",singleton_allele_fraction, doublet_allele_fraction,ref_count, alt_count, singleton_logprob, doublet_logprob);
                }
            }
...
fn load_allele_data(params: &Params, cell_clusters: &Vec<(usize, usize)>, num_clusters: usize) -> 
        (FnvHashMap<(usize,usize),Vec<(f64, f64)>>, FnvHashMap<(usize, usize), Vec<f64>>, Vec<f64>, Vec<Vec<(usize, u64, u64)>>) {
    ...
    for (index, (refline, altline)) in refs.lines().zip(alts.lines()).enumerate() {
        ...
        //println!("cell {} refcount {} altcount {} clust1 {} clust2 {}", cell, refcount, altcount, clust1, clust2);
        soup_allele_counts[locus].0 += refcount as f64;
        soup_allele_counts[locus].1 += altcount as f64;
        {   // to appease the borrow checker gods
            let mut locus_vec = cluster_allele_counts.get_mut(&(clust1, clust1)).unwrap();
