...
```
wget --load-cookies /tmp/cookies.txt "https://docs.google.com/uc?export=download&confirm=$(wget --quiet --save-cookies /tmp/cookies.txt --keep-session-cookies --no-check-certificate 'https://docs.google.com/uc?export=download&id=1woFmL_twfOEKxBSKuF5-yWfoCF1uxn9d' -O- | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1\n/p')&id=1woFmL_twfOEKxBSKuF5-yWfoCF1uxn9d" -O souporcell.sif && rm -rf /tmp/cookies.txt
```
```
